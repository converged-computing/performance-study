...
- It took 31 minutes to get a working cluster. 1 node only had 7/8 GPU. Deleting it didn't fix it - I had to scale up by 2 and then delete the offending node.
- Always refresh the interface and make sure (after issuing a command / clicking a button) it didn't log you out (and thus your issue didn't work).
...
Start a new set of times for size 32.

```bash
kubectl logs -n monitoring event-exporter-6bf9c87d4d-v4rtr -f  |& tee ./events-gpu-32-$(date +%s).json
```
...
oras push ghcr.io/converged-computing/metrics-operator-experiments/performance:aks-gpu-32-$app $output
```
```
...
# Size 32
# The fourth job got stuck - I cancelled at 5 minutes.
for i in $(seq 1 5); do     
...
for i in $(seq 1 5); do     
    echo "Running iteration $i"
    flux run --setattr=user.study_id=$app-32-iter-$i --env CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 -N32 -n 256 -g 1 /opt/mixbench/mixbench-cuda/wrapper 32
done
