...
/usr/bin/time -p mpirun -N 32 --map-by ppr:96:node -x UCX_POSIX_USE_PROC_LINK=n \
	/shared/bin/singularity exec --env UCX_TLS=all --env UCX_UNIFIED_MODE=y --env UCX_NET_DEVICES=mlx5_ib0:1 --env OPAL_PREFIX= \
