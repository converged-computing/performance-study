...
/usr/bin/time -p mpirun -n 1 -x UCX_POSIX_USE_PROC_LINK=n --map-by ppr:8:node \
    singularity exec --env UCX_TLS=ud_x,rc_mlx5,cma --env UCX_UNIFIED_MODE=y --env UCX_NET_DEVICES=mlx5_ib0:1 --env OPAL_PREFIX= \
     /shared/containers/mt-gemm_azure-hpc-gpu-ubuntu2204.sif /opt/gem/mt-dgemm.x 100 2048
